# Q001 Evidence Table

Last updated: 2026-09-23

この表は、Q001の主要Claimを判断するために重視した資料をまとめる。すべての関連研究を列挙するものではない。

| ID | Source | Design / population | Main result | Main limitation | Claim relevance |
|---|---|---|---|---|---|
| E001 | Galea et al. 2025, *Home-based shared book reading and developmental outcomes in young children* | Systematic review + meta-analyses. 46 studies, N=56,576; meta-analysis 28 studies. Mean age <4, not yet in compulsory schooling. Search: ERIC, PsycINFO, MEDLINE, EBSCOHost, Embase Ovid, Nov 2024. | Shared reading and language: r=0.381 (95% CI 0.289–0.474), 6 studies, N=1,311. Vocabulary: r=0.314 (0.291–0.336), 5 studies, N=6,629. Reading frequency and expressive vocabulary: r=0.259 (0.099–0.419), 5 studies, N=4,423. | Most contributing evidence is observational; majority of studies rated fair quality. Association cannot establish causality. Authors used an adaptation of GRADE rather than standard GRADE. | C001 strong support as association; does not establish C002 causality. |
| E002 | Dowdall et al. 2020, *Shared Picture Book Reading Interventions for Child Language Development* | Systematic review + meta-analysis of 19 RCTs, N=2,594, age 1–6 years. | Expressive language d=0.41; receptive language d=0.26; caregiver book-sharing competence d=1.01. | Heterogeneous interventions and measures. Meta-analysis supports an average intervention effect but does not show all forms of reading outperform active reading comparators. | C002 support; caregiver behaviour effect clearer than child-language effect. |
| E003 | Noble et al. 2019, *The impact of shared book reading on children's language skills* | Meta-analysis of shared-reading interventions. | Overall g=0.194 (95% CI 0.088–0.299). Passive-control studies g=0.263; active-control studies g=0.028 (95% CI -0.157–0.212). Follow-up g=0.139, p=.200. Publication-bias analyses suggested smaller corrected effects. | Results sensitive to control-group design; possible publication bias; many interventions had low dosage. | Qualifies C002; C003 and C005 remain uncertain. |
| E004 | Noble et al. 2020, *The Impact of Interactive Shared Book Reading on Children's Language Skills* | RCT, N=150, age 2y6m–3y0m; pause reading vs dialogic reading vs active shared-reading control; 6 weeks. | Intervention changed caregiver reading behaviour, but did not improve child language beyond active shared-reading control. | 6-week intervention; one age range; tests additional benefit over an already active reading condition, not reading vs no reading. | Direct evidence against a strong C003 claim. |
| E005 | Guevara et al. 2020, *Effects of Early Literacy Promotion on Child Language Development and Home Reading Environment* | RCT, N=120 Medicaid-eligible newborns; reading promotion from birth vs standard program starting at 6 months. | At 6 months, early-start group had richer home reading environment (SQRS 11.0 vs 9.4, p=.006), but PLS-5 language scores were similar. At 24 months there were no differences in SQRS or PLS-5. | Both groups received literacy promotion after 6 months, reducing contrast. Population was socioeconomically specific; 94/120 completed 24-month visit. | C004 not established; earlier start changed early reading environment but not measured language outcome. |
| E006 | Nakamura et al. 2026, *Impact of shared storybook reading on child development: The Japan Environment and Children's Study* | Nationwide Japanese prospective birth cohort; N=36,866 mother-child pairs. Reading measured repeatedly to age 3; ASQ-3 development outcomes. | Frequent reading at age 3 was associated with +5.5 ASQ-3 communication points (95% CI 5.0–6.0) vs rare reading after adjustment for parental background, play opportunities, media exposure and other covariates. | Observational association; residual confounding remains possible. 67,196 of 104,062 registered fetuses were excluded because required outcome/exposure/covariate data were incomplete. | Important Japan-specific support for C001; does not establish C002. |
| E007 | AAP 2024 Policy Statement + Technical Report | Professional guidance synthesizing early-literacy evidence. | Recommends shared reading beginning at birth and continuing at least through kindergarten; emphasizes language-rich interaction, relational health and school readiness. | Guideline recommendation integrates multiple outcomes and values; it is not a direct causal effect estimate for language alone. | Practical context; should not be used to inflate effect-size claims. |
| E008 | こども家庭庁 2025「乳幼児の遊びと体験」調査研究 | Japanese government-commissioned evidence summary. | Summarizes Dowdall meta-analysis: expressive language d=0.41, receptive d=0.26, caregiver reading competence d=1.01. | Secondary summary of E002 rather than independent primary evidence. | Japanese public-evidence context; not counted as an independent effect estimate. |
| E009 | Li & Pang 2025, *The effects of shared book reading on language and literacy development of ESL/EFL learners* | Three-level meta-analysis; 13 studies, 51 effect sizes, N=1,857 children learning English as a second/foreign language. | Short-term pooled effect g=0.49; long-term pooled effect g=0.20 and not statistically significant. | Population is narrower than Q001; language and literacy outcomes are combined; transportability to Japanese first-language families is limited. | Supports short-term intervention benefit in a subgroup and is consistent with C005 remaining uncertain. |
| E010 | *The effects of interactive reading on young children’s narrative abilities* (Frontiers in Psychology, 2025) | Meta-analysis of 25 experimental/quasi-experimental studies, N=2,886, mainly age 3–6. | Narrative ability g=0.425 (95% CI 0.333–0.518); I²=76.07%. | Narrative ability is not identical to Q001 primary language/vocabulary outcomes; intervention/control contrasts are heterogeneous and do not isolate ordinary shared reading as the comparator. | Context for C002/C003; does not establish superiority over ordinary shared reading. |
| E011 | Tang, Lau & Du 2026, *Effects and moderators of dialogic reading on children’s reading literacy* | Three-level meta-analysis; 64 studies, 260 effect sizes, N=10,463, preschool through primary school. | Overall reading-literacy effect g=0.76 with substantial heterogeneity; effects smaller on standardized measures and in younger children. | Broader age range and reading-literacy outcomes; not a direct estimate of additional spoken-language benefit over ordinary shared reading in toddlers/preschoolers. | Important update context for C003; does not overturn the active-control evidence used for the narrower claim. |

## Source links

- E001: https://doi.org/10.3389/flang.2025.1540562
- E002: https://pubmed.ncbi.nlm.nih.gov/30737957/
- E003: https://doi.org/10.1016/j.edurev.2019.100290
- E004: https://pubmed.ncbi.nlm.nih.gov/32539588/
- E005: https://pubmed.ncbi.nlm.nih.gov/37332625/
- E006: https://doi.org/10.1038/s41390-025-04721-7
- E007: https://doi.org/10.1542/peds.2024-069090
- E008: https://www.cfa.go.jp/assets/contents/node/basic_page/field_ref_resources/55e89c07-ffc5-40a9-bad7-ce6c6dd11b8a/90178a35/20250421_policies_kodomo_sodachi_research_05.pdf
- E009: https://doi.org/10.1080/1350293X.2025.2514768
- E010: https://doi.org/10.3389/fpsyg.2025.1653511
- E011: https://doi.org/10.1016/j.ijer.2026.102963

## Interpretation notes

### C001 — association

Supported.

The strongest recent synthesis of home-based exposure reports consistent positive associations between shared reading and language/vocabulary. The 2026 JECS result materially improves directness to Japanese families, while remaining observational.

### C002 — causal intervention effect

Partially supported.

RCT meta-analysis estimates favor reading interventions, but the estimated effect becomes much smaller when studies use active comparison groups. The evidence is compatible with a small average causal benefit, but does not justify a large-effect claim.

### C003 — special technique superiority

Uncertain.

Interactive/dialogic training clearly changes caregiver behaviour. Direct evidence that it improves child language beyond ordinary active shared reading is inconsistent. Recent 2025–2026 meta-analyses report benefits for adjacent outcomes or narrower populations, but they do not directly answer the same ordinary-shared-reading active-control contrast; therefore they do not justify upgrading C003 to supported.

### C004 — earlier is better

Insufficient for a causal claim.

Starting literacy promotion from birth rather than six months improved the early home reading environment in one RCT but did not produce a measurable language advantage at 24 months. Observational age-of-onset findings are vulnerable to confounding.

### C005 — long-term persistence

Uncertain.

The 2019 intervention meta-analysis did not find a statistically significant follow-up effect. Longer-term language outcomes remain insufficiently established.
